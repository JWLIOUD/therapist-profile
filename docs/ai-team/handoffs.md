...
- 發布結果：PR [#11](https://github.com/JWLIOUD/therapist-profile/pull/11) 已合併；main merge commit `13b9117b01fa477d6135e67f80b6145851b4ef85`；GitHub Pages run [`31930404211`](https://github.com/JWLIOUD/therapist-profile/actions/runs/31930404211) 為 `Success`，38 秒。
- 正式站驗證：sitemap HTTP 200 且 XML 通過；目標 `<lastmod>` 為 `2026-08-13`；34 個唯一 URL、0 個草稿 URL，34/34 URL 均為 HTTP 200；首頁、目標文章及 `robots.txt` 亦為 HTTP 200。
- 尚未完成：Search Console 重新讀取 sitemap、Google 重新檢索目標頁與搜尋成效觀察；這些有資料延遲，不能視為發布失敗。
- 已知風險：`lastmod` 是提示，不保證立即重抓、排名、流量或 Google 採用新版摘要。
- 需要使用者決定：無；已取得最終發布核准。
- 接手角色下一步：搜尋成效與索引驗收 AI 依每日工作流觀察 sitemap 上次讀取與目標頁最後檢索；不重複提交 sitemap 或 Request indexing。
- 接手完成條件：網站端發布條件已完成；外部 Google 狀態維持 `待觀察`，直到 Search Console 顯示新的讀取／檢索證據。

### 2026-08-17：單位合作優先 B 方案完成本地候選

- 任務 ID：`SITE-COLLAB-20260817-11`
- 目前狀態：`user_preview`
- 分支：`codex/collaboration-first-home-20260817`
- 交接來源：網站總管 AI、內容編輯 AI、前端維護 AI、SEO／發布維護 AI、品質檢查 AI、組織合作與講座邀約視角驗收 AI
- 接手角色：使用者；預覽確認後才交回網站總管與 SEO／發布維護 AI
- 使用者決策：選定 B 方案，授權實作到 push 前並要求本地預覽。
- 已完成：
  - 首頁改為心理健康講座、工作坊、團體及推廣活動合作優先。
  - 合作頁補強和諧粉彩、酒精墨水、團體／系列課程與推廣活動。
  - 全站正式頁主要導覽與 CTA 改為合作導向；LINE 降為少量次要聯絡入口。
  - 文章產生器、共用 CSS、metadata、JSON-LD 與 sitemap 同步。
  - 五重驗收 R0 → R2 完成，初審 FAIL 均有修改與複驗證據。
- 保持不變：文章與職場指南正文、案例、結論、來源、專業背景與合作經歷、canonical URL、robots 及草稿 noindex 邊界。
- 驗收證據：`docs/seo/keyword-research/collaboration-first-home-five-review-2026-08-17.md`。
- 本地結果：34/34 sitemap URL HTTP 200、34/34 self-canonical、25/25 文章正文保真、桌機／手機無水平溢位、console warning／error 為 0。
- 圖片決策：既有頭像、去背形象照與文章插畫足夠；不需啟動插畫設計 AI。
- 維護提醒：`tools/generate_articles.py` 已同步且 AST 語法 PASS；但此裝置缺少產生器指定的外部核准 Word 來源 `/專欄文章/專欄文章.docx`，未來重新生成前需先還原來源檔。現有 25 篇文章與 5 個系列頁已完成受控同步及正文保真驗證。
- 未完成：尚未 commit、push、PR、合併、發布或操作 Search Console。
- 本地預覽：`http://127.0.0.1:8013/index.html`、`http://127.0.0.1:8013/talks.html`。
- 使用者預覽重點：首页首屏的合作定位、合作頁活動段落、文章 CTA、LINE 降階是否符合期待。
- 下一步：使用者提出修改或明確核准；未核准前不得推送。若核准，再依發布前檢查做最終 diff、commit／push／PR 與正式站驗證。

### 2026-08-17：B 方案依使用者語氣與專欄標題回饋完成 R4 修正

- 任務 ID：`SITE-COLLAB-20260817-11`
- 目前狀態：`user_preview`
- 分支：`codex/collaboration-first-home-20260817`
- 交接來源：使用者、網站總管 AI、內容編輯 AI、前端維護 AI、SEO／發布維護 AI、品質檢查 AI、組織合作與講座邀約視角驗收 AI
- 接手角色：使用者；重新預覽確認後才交回網站總管
- R3 使用者回饋：多處「單位」使文案顯得制式、偏商務，沒有足夠的心理師專業與溫暖感；心理專欄 H1 過大，希望「不是心理百科，／而是陪你理解生活的故事。」維持兩行但縮小。
- R4 已完成：
  - 首頁主標改為「讓心理專業／貼近生活現場」。
  - 公開合作文案改以活動目標、參與者、場域與現場情境表達；受控公開檔案「單位」為 0。
  - 「客製合作／場域客製／設計合適方案」改為「一起設計合適內容／貼近現場／一起設計內容」。
  - 心理專欄 H1 改為固定兩行，桌機上限 44px，窄螢幕以 clamp 縮放。
  - 25 篇文章只同步合作 CTA 的引導短句，CTA 之前正文 25/25 與 `main` 一致；產生器同步。
- 五重複驗：文案、視覺／無障礙、架構／互動、SEO／索引邊界、組織合作／講座邀約視角均 PASS；完整證據見 `docs/seo/keyword-research/collaboration-first-home-five-review-2026-08-17.md`。
- 技術證據：34/34 sitemap 頁存在且有單一 H1／canonical／可解析 JSON-LD；7/7 FAQ 同步；首頁、專欄、合作頁本地實頁水平溢位均為 0；產生器語法與 `git diff --check` PASS。
- 保持不變：文章正文、案例、來源、專業背景、合作經歷、職場指南正文、URL、canonical、robots、sitemap 收錄邊界與草稿 noindex。
- 圖片決策：既有照片與插畫足夠，不需啟動插畫設計 AI。
- 未完成：未 commit、未 push、未開 PR、未合併、未發布、未操作 Search Console。
- 本地預覽：`http://127.0.0.1:8013/index.html`、`http://127.0.0.1:8013/articles.html`、`http://127.0.0.1:8013/talks.html`。
- 下一步：使用者重新預覽並提出修改或明確核准；核准前不得推送。

### 2026-08-17：B 方案 R4 取得正式發布核准

- 任務 ID：`SITE-COLLAB-20260817-11`
- 目前狀態：`ready_to_release`
- 使用者決策：已完成本地預覽，並明確回覆「好麻煩送出並且推送上線」。
- 核准版本：R4；五重驗收均 PASS，公開文案、心理專欄兩行標題與三個核心頁本地實頁均已複驗。
- 發布授權：可 commit、push、建立與合併 PR、等待 GitHub Pages，並執行正式站驗證。
- 範圍限制：只發布 `SITE-COLLAB-20260817-11` 相關網站與管理紀錄；排除每日／每週研究與 Search Console 報告等無關未追蹤檔。
- 接手角色：網站總管 AI、SEO／發布維護 AI。
- 下一步：提交候選、PR 合併、GitHub Pages 部署與正式站五重回歸；發布後另記錄 commit、PR、部署與線上證據。
