...
## 2026-08-17：單位合作優先首頁與全站導流（B 方案）

- [x] 建立需求單 `SITE-COLLAB-20260817-11` 與分支 `codex/collaboration-first-home-20260817`。
- [x] 將首頁主要目的調整為企業、校園、公部門、司法、社區與公益單位的心理健康講座、工作坊、團體與推廣活動合作。
- [x] 在合作頁補強和諧粉彩、酒精墨水、團體／系列課程與心理健康推廣活動，並明列手作活動不取代心理治療。
- [x] 將 34 個正式頁的主要導覽與 CTA 改為合作導向；官方 LINE 只保留 4 個次要聯絡入口。
- [x] 同步文章產生器、共用導覽 CSS、metadata、JSON-LD 與 sitemap 34 筆 `lastmod=2026-08-17`。
- [x] 25 篇文章正文排除 CTA 後與 main 比對，內容變更為 0/25。
- [x] 完成文案、風格、架構、SEO、組織合作五重驗收；R0／R1 發現的首屏、手機按鈕與合作入口順序問題均於 R2 複驗 PASS。
- [x] 記錄 R3 使用者預覽 FAIL：公開文案重複「單位」、首頁語氣偏商務、心理專欄 H1 過大。
- [x] 完成 R4 文案與版面修正：公開受控文案「單位」0 筆；首頁改為「讓心理專業／貼近生活現場」；心理專欄 H1 縮小並固定兩行。
- [x] 完成 R4 五重複驗：34/34 正式頁靜態檢查、25/25 文章正文保真、7/7 FAQ 同步、三個核心本地實頁無水平溢位。
- [x] 本地 34/34 sitemap URL HTTP 200、34/34 self-canonical、內部連結／fragment／JSON-LD 無錯誤，console warning／error 為 0。
- [x] 使用者重新預覽 `http://127.0.0.1:8013/index.html`、`http://127.0.0.1:8013/articles.html` 與 `http://127.0.0.1:8013/talks.html`。
- [x] 使用者已於 2026-08-17 明確核准送出並推送上線。
- [ ] 提交候選、推送分支、建立並合併 PR，確認 GitHub Pages 部署成功。
- [ ] 發布後驗證正式首頁、心理專欄、講座頁、代表文章、導覽、metadata、sitemap 與 console。
- [ ] 發布後依五重驗收與 Search Console 工作流執行正式站驗證及 7／28／90 天成效回饋。
